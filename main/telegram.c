...
#define FORM_DATA_PREFIX_LENGTH FORM_DATA_BOUNDARY_LENGTH + 114
#define FORM_DATA_SUFFIX_LENGTH FORM_DATA_BOUNDARY_LENGTH + TELEGRAM_CHAT_ID_LENGTH + FORM_DATA_BOUNDARY_LENGTH + 64

static const char *CONTENT_TYPE_HEADER = "multipart/form-data; boundary=" FORM_DATA_BOUNDARY;
static const char *URL = "https://api.telegram.org/bot" TELEGRAM_TOKEN "/sendPhoto";
...
void construct_payload(uint8_t *body, size_t body_len, char **out, size_t *out_len)
{
  ...
  size_t offset = 0;
  memcpy(payload + offset, FORM_DATA_PREFIX, FORM_DATA_PREFIX_LENGTH);
  offset += FORM_DATA_PREFIX_LENGTH;
  memcpy(payload + offset, body, body_len);
  offset += body_len;
  memcpy(payload + offset, FORM_DATA_SUFFIX, FORM_DATA_SUFFIX_LENGTH);
